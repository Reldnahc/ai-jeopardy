...
- Completed: extracted lobby start selector, preload-ack, and game-ready helpers out of `lobbyStartHandlers.ts`.
...
- Completed so far:
  ...
  - Extracted lobby start selector choice, preload acknowledgement tracking, and game-ready phase helpers into `backend/lobby/startFlow.ts`.
...
- Completed so far:
  ...
  - Centralized lobby start selector/preload/game-ready state transitions into `backend/lobby/startFlow.ts`.
