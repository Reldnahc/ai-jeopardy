...
export const gameHandlers: Record<string, WsHandler> = {
  ...
  buzz: async ({ ws, data, ctx }: WsHandlerArgs<BuzzData>) => {
    ...
    if (!game.pendingBuzz) {
      ...
      game.pendingBuzz.timer = setTimeout(async () => {
        ...
        ctx.fireAndForget(
          ctx.repos.profiles.incrementTimesBuzzed(winner.playerUsername),
        ...
        setTimeout(() => {
          ...
          const ANSWER_SECONDS =
            typeof gg.timeToAnswer === "number" && gg.timeToAnswer > 0 ? gg.timeToAnswer : 9;
  ...
  "dd-snipe-next": async ({ data, ctx }: WsHandlerArgs<DdSnipeNextData>) => {
  ...
  "answer-audio-blob": async ({ ws, data, ctx }: WsHandlerArgs<AnswerAudioBlobData>) => {
    ...
    try {
      ...
      verdict = (
        await ctx.judgeClueAnswerFast(expectedAnswer, transcript, game.selectedClue.question)
      ).verdict;
    } catch (e) {
  ...
  "mark-all-complete": async ({ data, ctx }: WsHandlerArgs<GameIdData>) => {
    const { gameId } = data;
    const game = ctx.games[gameId];
    if (!game) return;
  ...
  "update-score": async ({ data, ctx }: WsHandlerArgs<UpdateScoreData>) => {
  ...
  "submit-wager": async ({ data, ctx }: WsHandlerArgs<SubmitWagerData>) => {
  ...
  "submit-drawing": async ({ data, ctx }: WsHandlerArgs<SubmitDrawingData>) => {
