...
export function selectProvider(req: TtsRequest): {
  provider: TtsProvider;
  effectiveReq: TtsRequest;
} {
  const providers = getProviders();
  ...
  if (effectiveReq.provider) {
    ...
    if (!p.supports(effectiveReq))
      throw new Error(`Provider ${p.name} does not support this request`);
