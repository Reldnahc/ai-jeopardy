...
export async function generateCategoryPoolFromAi(opts: {
  count: number;
  model?: string;
  prompt?: string;
}): Promise<string[]> {
  ...
  const raw = await callAiJson(model, prompt);
  const parsed = parseAiJson<CategoryPoolResponse>(raw);
  ...
  if (out.length === 0) {
    throw new Error("AI model returned no categories.");
  }
