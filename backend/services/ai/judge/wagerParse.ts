...
export function parseWagerDeterministic(transcriptRaw: string, maxWager: number): WagerParseResult {
    const transcript = String(transcriptRaw || "").trim().toLowerCase();
...
export async function parseDailyDoubleWager(args: {
    ...
    if (wager === null) {
        return { wager: null, reason: reason ?? "no-number", confidence, transcript: transcript.toLowerCase() };
    }

    if (wager < 0) {
        return { wager: null, reason: "negative", confidence: Math.min(confidence, 0.9), transcript: transcript.toLowerCase() };
    }

    if (wager > maxWager) {
        return { wager: null, reason: "too-high", confidence: Math.min(confidence, 0.9), transcript: transcript.toLowerCase() };
    }
