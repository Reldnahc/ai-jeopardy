...
export const validateImportedBoardData = (boardData) => {
    ...
    if (Array.isArray(fjCats)) {
        if (fjCats.length < 1) return { ok: false, error: "finalJeopardy.categories must have at least 1 category." };
        fjCats = fjCats[0];
    }
    ...
    if (!fj || typeof fj !== "object") return { ok: false, error: "finalJeopardy.categories.values[0] must be an object." };
    if (!isNonEmptyString(fj.question)) return { ok: false, error: "finalJeopardy.categories.values[0].question must be a non-empty string." };
    if (!isNonEmptyString(fj.answer)) return { ok: false, error: "finalJeopardy.categories.values[0].answer must be a non-empty string." };
