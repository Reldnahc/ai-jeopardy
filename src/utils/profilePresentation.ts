  ...
  switch (preset) {
    ...
    case "glow":
      return {
        ...
        borderStyle: "solid",
        borderColor: color,
        boxShadow: isHexColor(colorRaw) ? `0 0 0.8rem ${color}` : "0 0 0.8rem rgba(0,0,0,0.35)",
      };
...
const FONT_CLASS_MAP: Record<string, string> = Object.fromEntries(
...
export function getProfilePresentation(args: {
  ...
  const nameStyle = isHexColor(nameColor)
